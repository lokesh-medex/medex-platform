...
/**
 * Animated AI launcher button — pulsing glow ring + slow-rotating partial
 * ring around a two-spark gradient mark, extracted from the standalone
 * "AI Launcher Icon" design export and rescaled to sit next to NavSearch's
 * h-9.5/h-9 circular button. White chrome — matches the logo chip and the
 * tooltip, and reads as a "chip" among the navbar's translucent glass
 * buttons rather than blending into them, since the animated glow and
 * gradient mark already carry the color. `onClick` is a placeholder until
 * the AI chat feature it launches exists.
 */
...
interface AiLauncherIconProps {
  /** Accepted for API parity with NavSearch; the mark stays full-size on mobile rather than shrinking. */
  compact?: boolean;
  onClick?: () => void;
}

export default function AiLauncherIcon({ onClick }: AiLauncherIconProps) {
