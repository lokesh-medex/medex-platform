...
function Card({ v }: { v: Vendor }) {
  return (
    <div className="rounded-2xl overflow-hidden bg-white border border-slate-200 flex flex-col transition-[transform,box-shadow] duration-200 hover:-translate-y-1 hover:shadow-[0_16px_32px_#0f172a1a]">
      <div className="relative h-[140px] overflow-hidden" style={{ background: v.gradient }}>
        <Image src={v.img} alt={v.name} fill sizes="(min-width: 1040px) 25vw, 50vw" className="object-cover" />
      </div>
      <div className="p-4 flex flex-col flex-1">
        <h4 className="font-[family-name:var(--font-heading)] text-[#0f172a] font-bold text-[15px] mb-2">{v.name}</h4>
        <div className="flex items-start gap-1.5 mb-4 flex-1">
          <FiMapPin size={14} color="#64748b" className="shrink-0 mt-0.5" />
          <p className="text-slate-500 text-xs leading-[1.4] m-0">{v.location}</p>
        </div>
        <button className="self-start text-[12.5px] py-2 px-4 rounded-full text-white font-bold border-none cursor-pointer font-sans bg-[linear-gradient(120deg,#f33b27,#872888)]">
          Book Now
        </button>
      </div>
    </div>
    ...
    <section id="vendors" className="py-20 bg-white">
      <div className="max-w-[1280px] mx-auto px-5 dt:px-8">
        ...
        <div className="hidden sm:block text-center max-w-[560px] mx-auto mb-10">
          <span className="text-[#872888] font-bold text-sm font-sans">Our Network</span>
