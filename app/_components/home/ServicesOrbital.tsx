...
import { SERVICES_DATA, type ServiceNode } from "@/app/_lib/homepage-data";
...
export default function ServicesOrbital() {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      setOrbitAngle((prev) => (activeService === null ? (prev + ORBIT_STEP_DEG) % 360 : prev));
    }, ORBIT_INTERVAL_MS);
  ...
  const activeSvcData = SERVICES_DATA.find((s) => s.id === activeService) ?? null;
  const relatedIds = activeSvcData ? activeSvcData.relatedIds : [];

  return (
    <section id="services" className="pt-20 pb-20 bg-[#1a0320] relative overflow-x-hidden">
      <div className="max-w-[1280px] mx-auto px-5 dt:px-8 relative">
        <div className="text-center max-w-[560px] mx-auto mb-4">
          <span className="text-[#f472a0] font-bold text-sm font-sans">What Medex aggregates</span>
          <h2 className="font-[family-name:var(--font-heading)] text-white font-bold text-[clamp(26px,3.2vw,36px)] tracking-[-0.02em] mt-2 mb-3 text-balance">
            One network, every service you need
          </h2>
          <p className="text-white/60 text-base m-0 font-sans">
            Select a node to see which vendors sit behind each service. Related services pulse together.
          </p>
        </div>
        ...
        >
          <svg className="absolute inset-0 w-full h-full pointer-events-none" viewBox="0 0 620 620">
            {SERVICES_DATA.map((svc, i) => {
              ...
              const lineColor = isActive ? iconColor : isRelated ? "rgba(255,255,255,0.4)" : "rgba(255,255,255,0.12)";
              return (
                <line
                  key={svc.id}
                  x1={CENTER}
            ...
            <circle cx={CENTER} cy={CENTER} r="210" fill="none" stroke="rgba(255,255,255,0.12)" strokeWidth="1" />
          </svg>
          ...
          <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[120px] h-[120px] rounded-full bg-white flex items-center justify-center z-20 shadow-[0_8px_24px_#00000033]">
            ...
            <Image src="/medex.webp" alt="Medex" height={32} width={140} className="h-8 w-auto max-w-[80%] object-contain" />
          </div>
          ...
          {SERVICES_DATA.map((svc, i) => {
            ...
            const iconColor = ICON_COLORS[svc.icon];
            const bg = isActive ? "#fff" : isRelated ? "rgba(255,255,255,0.15)" : "rgba(255,255,255,0.05)";
            const borderColor = isActive ? iconColor : isRelated ? "rgba(255,255,255,0.5)" : "rgba(255,255,255,0.15)";
            const shadow = isActive ? `0 0 24px ${iconColor}80` : "none";
            const scale = isActive ? "scale(1.15)" : dim ? "scale(0.92)" : "scale(1)";
            const glow = isActive ? `${iconColor}40` : "transparent";
            const glowSize = isActive ? 140 : 0;
            const renderIconColor = isActive ? "#0f172a" : iconColor;
            const labelColor = isActive ? "#fff" : isRelated ? "rgba(255,255,255,0.85)" : "rgba(255,255,255,0.5)";
            ...
            return (
              <div
                ...
                style={{ left: `${cxPct}%`, top: `${cyPct}%`, transform: "translate(-50%,-50%)", zIndex: isActive ? 30 : 10 }}
              >
                <div className="relative flex flex-col items-center">
                  ...
                  />
                  <button
                    onClick={() => setActiveService((prev) => (prev === svc.id ? null : svc.id))}
                    ...
                    style={{ background: bg, border: `2px solid ${borderColor}`, boxShadow: shadow, transform: scale }}
                  >
                ...
                {isActive && (
                  <div className="absolute top-[112px] left-1/2 -translate-x-1/2 w-[264px] rounded-xl p-4.5 text-left bg-[rgba(10,14,24,0.96)] backdrop-blur-md border border-white/15 shadow-[0_24px_48px_#00000066] z-40">
                    <div className="flex items-center justify-between mb-2.5">
                      <span className="text-[10px] font-bold tracking-[0.08em] font-sans" style={{ color: iconColor }}>
                        {svc.title.toUpperCase()}
                      </span>
                    </div>
                    <p className="text-white/75 text-[13px] leading-[1.5] mb-3.5 font-sans">{svc.blurb}</p>
                    <div className="grid grid-cols-2 gap-2.5 mb-3.5">
                      <div className="bg-white/[0.06] rounded-lg p-2.5">
                        <div className="font-[family-name:var(--font-heading)] text-white font-bold text-lg">{svc.metricValue}</div>
                        <div className="text-white/50 text-[10.5px] mt-0.5 font-sans">{svc.metricLabel}</div>
                      </div>
                      <div className="bg-white/[0.06] rounded-lg p-2.5">
                        <div className="font-[family-name:var(--font-heading)] text-white font-bold text-lg">{svc.secondaryValue}</div>
                        <div className="text-white/50 text-[10.5px] mt-0.5 font-sans">{svc.secondaryLabel}</div>
                      </div>
                    </div>
                    <div className="border-t border-white/10 pt-3">
                      <span className="text-white/45 font-bold text-[10px] tracking-[0.06em] font-sans">TOP VENDORS</span>
                      <div className="flex flex-wrap gap-1.5 mt-2">
                        {svc.vendors.map((v) => (
                          <span key={v} className="text-[11px] rounded-full py-1 px-2.5 bg-white/[0.08] text-white/85 border border-white/10 font-sans">
                            {v}
                          </span>
                        ))}
                      </div>
