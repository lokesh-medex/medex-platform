...
function buildColumn(colIndex: number): { anim: string; duration: string; items: ColumnItem[] } {
  const offset = colIndex % TESTIMONIALS_DATA.length;
  const base = [...TESTIMONIALS_DATA.slice(offset), ...TESTIMONIALS_DATA.slice(0, offset)];
  const doubled = [...base, ...base];
  const items: ColumnItem[] = doubled.map((t, idx) => ({
    ...t,
    key: `${colIndex}-${idx}`,
    stars: [1, 2, 3, 4, 5].map((n) => (n <= t.rating ? "#f33b27" : "#e2e8f0")),
  }));
  return {
    ...
    <section id="testimonials" className="py-20 bg-[#F5F5F5] overflow-hidden">
      <div className="max-w-[1280px] mx-auto px-5 dt:px-8">
        <div className="text-center max-w-[560px] mx-auto mb-12">
          <span className="text-[#872888] font-bold text-sm font-sans">From people who&apos;ve booked</span>
          <h2 className="font-[family-name:var(--font-heading)] text-[#0f172a] font-bold text-[clamp(26px,3.2vw,36px)] tracking-[-0.02em] mt-2 mb-6">
            What patients say
          </h2>
          <div className="inline-flex items-center gap-3 rounded-2xl py-3 px-5 bg-white border border-slate-200">
            ...
            <div className="text-left">
              <div className="flex items-baseline gap-2">
                <span className="font-[family-name:var(--font-heading)] text-[#0f172a] font-bold text-[22px]">4.4</span>
                <div className="flex gap-px">
                  {[1, 1, 1, 1, 0].map((filled, i) => (
                    <FaStar key={i} size={13} color={filled ? "#f33b27" : "#e2e8f0"} />
                  ))}
                </div>
              </div>
              <span className="text-slate-500 text-xs font-sans">Based on 642 Reviews</span>
            </div>
          </div>
        </div>
      </div>
          ...
          {columns.map((col, i) => (
            ...
            >
              <div
                ...
                style={{ animation: `${col.anim} ${col.duration} linear infinite` }}
              >
                {col.items.map((t) => (
                  <div key={t.key} className="rounded-[14px] p-4.5 bg-white border border-slate-200 shadow-[0_8px_20px_#0f172a12] shrink-0">
                    <div className="flex items-center gap-2.5 mb-3">
                      <InitialsAvatar name={t.name} className="h-[34px] w-[34px] shrink-0 text-sm" />
                      <div>
                        <div className="text-[#0f172a] font-bold text-[13px] font-sans">{t.name}</div>
                        <div className="text-slate-500 text-[11px] font-sans">{t.role}</div>
                      </div>
                    </div>
                    ...
                    <p className="text-[#334155] text-[12.5px] leading-[1.5] m-0 font-sans">&quot;{t.quote}&quot;</p>
                  </div>
                ))}
              </div>
            </div>
