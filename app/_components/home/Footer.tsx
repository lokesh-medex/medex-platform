...
export default function Footer() {
  return (
    <footer className="relative overflow-hidden bg-[#0d0116] py-14 pb-7 font-sans">
      <Mesh preset="footer" />
      <BackdropMotifs
        count={5}
        opacity={0.05}
          ...
          <div className="flex flex-wrap items-center gap-3">
            {PAYMENT_PARTNERS_DATA.map((pp) => (
              <Tag
                key={pp}
                variant="filled"
                className={`m-0! rounded-lg border-0! px-3! py-1.5! text-xs font-bold text-white/80! ${glass.dark}`}
              >
                {pp}
              </Tag>
            ))}
