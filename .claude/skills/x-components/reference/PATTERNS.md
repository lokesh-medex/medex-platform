...
const makeActionItems = (content: string, onRetry: () => void) => [
  ...
  },
  {
    key: "feedback",
    actionRender: () => {
      const [val, setVal] = React.useState<"like" | "dislike" | "default">(
        "default"
      );
...
const roles = {
  assistant: {
    ...
    contentRender: (content, info) => {
      const { reasoning, answer } = content as {
        reasoning: string;
        answer: string;
      };
      return (
...
<Sender
  prefix={
    <PaperClipOutlined
      onClick={() => attachRef.current?.select({ multiple: true })}
    />
  }
  header={
    fileList.length > 0 && (
...
const commands = [
  {
    value: "/search",
    label: "/search — Search the web",
    icon: <SearchOutlined />,
  },
  {
    value: "/summarize",
    label: "/summarize — Summarize text",
    icon: <FileOutlined />,
  },
