  ...
  return (
    <>
      ...
      <Sender
        ...
        onCancel={abort}
        onSubmit={(val) =>
          onRequest({ query: val, role: "user", stream: false })
        }
      />
    </>
  );
...
class AttachmentProvider extends AbstractChatProvider<
  MyMessage,
  { query: string },
  MyOutput
> {
  transformParams(
    ...
    options: XRequestOptions<any, any, any>
  ) {
